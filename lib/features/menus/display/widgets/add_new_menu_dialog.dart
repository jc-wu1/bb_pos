...
class _AddNewMenuDialogViewState extends State<AddNewMenuDialogView> {
  ...
  Future<void> _onUploadImagePressed() async {
    ...
    _imageController.setLoading();
    _imageController.setImage(File(image.path));
    _imageController.stopLoading();
  }
...
class CategoriesSelector extends StatelessWidget {
  ...
  static Future<void> onAddNewCategoryPressed(
    ...
    final dialogResult = await showDialog<CategoryItem>(
      ...
      builder: (context) {
        return Dialog(
          constraints: const BoxConstraints(maxWidth: 560, minHeight: 280),
  ...
  static Future<void> onModifyCategoryPressed(
    BuildContext context, {
    ...
    if (!context.mounted) return;
    context.read<CategoriesBloc>().add(
      CategoryModified(categoryId: categoryId, categoryItem: dialogResult),
    );
  }
